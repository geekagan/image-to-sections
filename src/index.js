 ...
 * @version 1.0.0
 * @description: 图片处理工具库
 * @introduction: 专注于前端图片处理核心需求，API 简洁易懂，开箱即用，无需复杂配置，体积轻量，不增加项目冗余。
 * @license MIT
 * @copyright 2022 geekagan
 * @email geekagan@163.com
 */
/**
...
export async function getBigImageSectionFiles(imgFile, options = {}) {
	...
	const canvasSections = getImageCanvasSections(loadedImg, {
		sectionWidth: options.sectionWidth,
		sectionHeight: options.sectionHeight,
		cutDirection: options.cutDirection,
	});
	...
	const imageFilePms = canvasSections.map((canvasItem, index) =>
		canvasToImageFile(canvasItem, { imageName: `section-${index}`, transType: "image/png" }),
	);
...
export function zoomImageForSection(loadedImg, options = {}) {
	...
	switch (options.cutDirection) {
		case "horizontal":
			...
			loadedImg.height = Math.round(
				(loadedImg.naturalHeight * loadedImg.width) / loadedImg.naturalWidth,
			);
			break;
		case "vertical":
...
export function getImageCanvasSections(loadedImg, options) {
	...
	const canvasSections =
		options.cutDirection === "vertical"
			? getImageCanvasSectionsV(loadedImg, options.sectionWidth)
			: getImageCanvasSectionsH(loadedImg, options.sectionHeight);
	...
	if (imgHeight <= cHeight) {
		...
		for (let i = 1; i <= sectionCount; i++) {
			...
			ctx.drawImage(
				...
				sy,
				imgNaturalWidth,
				canvas.height * sectionZoomRatio,
				0,
				0,
				canvas.width,
				canvas.height,
...
const imageLoaded = async (newImg) =>
	new Promise((resolve) => (newImg.onload = () => resolve(newImg)));
...
export function imageToCanvas(loadedImg, options = {}) {
	...
	if (!options.canvWidth && !options.canvHeight) {
		...
		if (!options.distorted)
			options.canvHeight = options.canvHeight = (imgHeight / imgWidth) * options.canvWidth;
	}
...
export async function imageFileToThumbFile(imgFile, options = {}) {
	...
	const thumbCanvas = imageToCanvas(loadedImg, {
		canvWidth: options.thumbWidth,
		canvHeight: options.thumbHeight,
		distorted: false,
	});
	...
	thumbFile = await canvasToImageFile(thumbCanvas, {
		imageName: "thumb-image",
		transType: "image/png",
	});
	return thumbFile;
}
...
export async function canvasToImageFile(canvasObj, options = {}) {
	...
	const promise = new Promise((resolve) => {
		canvasObj.toBlob(
			(blob) => {
				const imageFile = new File([blob], `${options.imageName}.${options.suffix}`, {
